...
async function mockReview({ persona, product }) {
  ...
  const samples = {
    hausa: {
      rating: 4,
      review: `This ${product.name} na correct one for ${persona.crop}. I use am for my ${persona.farmSize} hectare farm last season for ${persona.state}. The result good but price don go up too much this year. Wallahi, ${product.price ? `₦${product.price.toLocaleString()}` : 'the price'} is heavy o. If you get the money buy am, but try mix am with manure to stretch am.`,
      language: 'hausa-english',
      reasoning: `Farmer is from ${persona.state} (Hausa speaking region), grows ${persona.crop}, uses ${persona.fertilizerType} fertilizer. Price sensitive given farm size of ${persona.farmSize} hectares. Pidgin and Hausa expressions woven in.`,
    },
    yoruba: {
      rating: 3,
      review: `I tried this ${product.name} for my ${persona.crop} farm. E no bad, but I expect more for the money wey I pay. My soil for ${persona.state} dey ${persona.soilType}, and the product no really match am well. Next season I go try another brand. For ${persona.farmSize} hectares, e too cost.`,
